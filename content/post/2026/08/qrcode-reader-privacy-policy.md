...
ただし、**これらの操作で削除されるのは端末内のデータのみです**。上記のとおり Google アナリティクス for Firebase に送信済みのデータは、この操作では削除されません。送信済みのデータの削除をご希望の場合は、本ページの「お問い合わせ」に記載の窓口までご連絡ください。
...
**Google アナリティクス for Firebase に送信済みのデータ（QR コードの内容および名前の先頭 24 文字を含む）は、上記の操作では削除されません。** 削除をご希望の場合は、下記の「お問い合わせ」に記載の窓口までご連絡ください。ご連絡の際は、お使いの端末の機種名と、削除をご希望の内容をお知らせください。
