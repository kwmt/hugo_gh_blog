...
=====================================================================
  ...
  【要確認】問い合わせ窓口（連絡先）を記載していない
    ...
    ※ QR コードの内容の先頭 24 文字を Firebase Analytics に送信している
      ことが判明したため、連絡先の要否は再検討したほうがよい。送信済みの
      データはアプリ内の操作では削除できず、現状ユーザーが削除を依頼する
      手段が本ポリシー上に存在しない。
...
ただし、**これらの操作で削除されるのは端末内のデータのみです**。上記のとおり Google アナリティクス for Firebase に送信済みのデータは、この操作では削除されません。
