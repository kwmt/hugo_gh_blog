...
=====================================================================
  ...
  【注意】データ削除リクエストの窓口は意図的に記載していない
    Play のアカウント削除要件はアプリ内でアカウントを作成できる場合の要件で、
    本アプリは対象外のため、削除リクエストの窓口は設けない方針とした。
    そのため、データセーフティ申告の「ユーザーはデータの削除をリクエスト
    できますか」には「いいえ」で回答し、記事と食い違わないようにすること。

    方針を変えて窓口を設ける場合は、Google の User Deletion API で
    APP_INSTANCE_ID を指定して削除する形になるが、ユーザーが自分の
    App-instance ID を知る手段をアプリ側に用意する必要がある。
